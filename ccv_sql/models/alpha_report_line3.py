...
class AlphaReportLine3(models.TransientModel):
    _name = 'alpha.report.line3'
    _description = 'Chi tiết công nợ phải thu'

    parent_id = fields.Many2one('alpha.report', string="Thông tin chung")
    team_id = fields.Many2one('crm.team', string="Đội bán hàng", related="partner_id.team_id", store=True)
    date = fields.Date()
    ...
    end_w_tax_debit = fields.Float(string="Nợ cuối kỳ", digits=(16, 0))
    end_w_tax_credit = fields.Float(string="Có cuối kỳ", digits=(16, 0))
    currency_id = fields.Many2one("res.currency", string="Tiền tệ", related="partner_id.currency_id")
    # Khu vực hóa đơn
