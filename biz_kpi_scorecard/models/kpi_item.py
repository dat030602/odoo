...
class kpi_item(models.Model):
    ...
    @api.depends('category_id','category_id.employee_id')
    def _compute_employee_id(self):
        for res in self:
            res.employee_id = res.category_id.employee_id if res.category_id and res.category_id.employee_id else False
    ...
    def _get_id_from_string(self, str_id):
         ...
         * int
        """
        all_numbers = re.findall(r'\d+', str_id)
    ...
    def _return_formated_appearance(self, value, novalue_change=False, currency=False):
        ...
        if isinstance(formatted_value, (int, float)):
            if abs(formatted_value) > 100000:
                # Format theo currency của công ty nếu có
                currency_obj = self.env.company.currency_id
                formatted_value = currency_obj and currency_obj.format(formatted_value) or "{:,.3f}".format(formatted_value)
            else:
                formatted_value = "{:,.3f}".format(formatted_value)
