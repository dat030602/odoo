...
export class CameraWidget extends Component {
    setup() {
        if(this.record && this.record.resModel === "res.partner") this.type = "checkin";
        else if(this.props.rawAttrs && this.props.rawAttrs.type) this.type = this.props.rawAttrs.type;
        else this.type = false;
        this.record = this.props.record
        this.recordData = this.record.data
    }
    _onCheckin(ev){
        if (!this.record.data.partner_id){
            Dialog.alert(this, _t('Please select partner'), {
                title: _t("Error"),
            });
            return
        }
        if (!this.record.data.is_valid_check){
            var error_name = _t('Customer ') + this.record.data.valid_partner + _t(' have not completed check-in/check-out')
        ...
        var partner_id = self.record.data.partner_id[0]
                        ...
                        },
                        {
                            ...
                            click:  function () {
                                ...
                                return rpc.query({
                                    ...
                                    args: [parseInt(self.record.data.id), img_data_base64, lat, long],
    ...
    _onCheckout(ev){
        ...
        if (!this.record.data.sale_route_report_id){
            var error_name = _t('Customer ') + this.record.data.partner_id[1] + _t(" haven't done the check-in yet")
            Dialog.alert(this, error_name , {
                title: _t("Error"),
            });
            return
                        ...
                        },
                        {
                            ...
                            click:  function () {
                                ...
                                return rpc.query({
                                    ...
                                    args: [parseInt(self.record.data.id),parseInt(self.record.data.id), img_data_base64, lat, long],
                                }).then(async function(result) {
                                    if (result){
    ...
    _onCheckinPartner(ev){
        ...
        var partner_id = _this.record.data.id
                        ...
                        },
                        {
                            ...
                            click:  function () {
                                ...
                                return rpc.query({
                                        model: 'res.partner',
                                        method: 'generate_checkin_history',
                                        args: [partner_id, img_data_base64, lat, long],
                                    }).then(function(check_coordinates_received) {
                                        location.reload();
                                });
                            }
...
CameraWidget.template = "biz_ccv_sale.CameraWidget";
registry.category("view_widgets").add("camera_widget", CameraWidget);
