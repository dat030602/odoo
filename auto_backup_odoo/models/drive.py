...
class OdooBackupDrive(models.Model):
    ...
    def upload_file(self, path, file, x_days_ago, database_name):
        ...
        self.log('%s, %s, %s' % (response.status_code, UPLOAD_URL, payload))
        ...
        for database_name in database_list:
            ...
            if dump_dir:
                ...
                for file in glob_list:
                    status = self.upload_file(path=dirr, file=file, x_days_ago=x_days_ago, database_name=database_name.strip())
                    self.log("%s dump status" % status)
                    if status in ['Invalid URL', 'Invalid File']:
                        self.log("Invalid File / URL", "Exception")
                        self.message_post(
