...
TITLE = 'SỐ CHI TIẾT BÁN HÀNG SẢN PHẨM HUMIC & PHÂN BÓN LÁ THEO KHU VỰC'


COLUMNS = [
    {"size": 8,  "name": 'STT'                   , 'field':"no"                     , "type": 'text_center'       , "sum": False  },
    {"size": 12, "name": 'Khu vực'               , 'field':"region_name"            , "type": 'text_center'       , "sum": False  },
    {"size": 12, "name": 'Ngày hạch toán'        , 'field':"accounting_date"        , "type": 'date'              , "sum": False  },
    {"size": 25, "name": 'Số chứng từ'           , 'field':"voucher_number"         , "type": 'text_center'       , "sum": False  },
    {"size": 15, "name": 'Số hóa đơn'            , 'field':"invoice_number"         , "type": 'text_center'       , "sum": False  },
    ...
    {"size": 20, "name": 'Mã hàng'               , 'field':"product_code"           , "type": 'text_center'       , "sum": False  },
    {"size": 50, "name": 'Tên hàng'              , 'field':"product_name"           , "type": 'text'              , "sum": False  },
    {"size": 10, "name": 'ĐVT'                   , 'field':"uom_name"               , "type": 'text_center'       , "sum": False  },
    {"size": 12, "name": 'Số lượng tấn'          , 'field':"quantity_ton"           , "type": 'number_float2'     , "sum": True   },
    {"size": 15, "name": 'Đơn giá'               , 'field':"unit_price"             , "type": 'number_int'        , "sum": False  },
    {"size": 18, "name": 'Doanh số sau thuế'     , 'field':"total_amount"           , "type": 'number_int'        , "sum": True   },
    {"size": 18, "name": 'Doanh thu tiền'        , 'field':"amount_paid"            , "type": 'number_int'        , "sum": True   },
    {"size": 15, "name": 'Tiền hoa hồng'         , 'field':"commission_amount"      , "type": 'number_int'        , "sum": True   },
    {"size": 18, "name": 'Hoa hồng nhân viên'    , 'field':"employee_commission_amount", "type": 'number_int'     , "sum": True   },
    {"size": 20, "name": 'Người Phụ Trách'       , 'field':"person_in_charge"       , "type": 'text_center'       , "sum": False  },
    {"size": 20, "name": 'Ghi Chú'               , 'field':"note"                   , "type": 'text_center'       , "sum": False  },
]
...
class RpHumicSalesDetailReport(models.AbstractModel):
    _name = "report.biz_kpi_business_salary.rp_humic_sales_detail"
    ...
    def _get_data_export(self, obj):
        """Lấy dữ liệu từ salary.humic.sales.detail và nhóm theo khu vực"""
        lines_data = obj.humic_sales_detail_ids.sorted(lambda line: (line.team_id.id or 0, line.partner_id.name or '', line.product_id.name or ''))
        ...
        for line in lines_data:
            ...
            # Lấy thông tin từ invoice để có ngày hạch toán và số chứng từ
            accounting_date = None
            voucher_number = ''
            invoice_number = line.invoice_name or ''
            person_in_charge = line.user_id.name or ''
            note = ''
            
            if line.invoice_ids:
                # Lấy ngày hạch toán từ invoice (invoice_date hoặc date)
                first_invoice = line.invoice_ids[0]
                accounting_date = first_invoice.invoice_date or first_invoice.date
                
                # Kiểm tra nếu hóa đơn đã thanh toán để ghi chú
                if first_invoice.payment_state == 'paid':
                    note = 'Đã TT'
                elif first_invoice.payment_state == 'partial':
                    note = 'TT một phần'
                
                # Lấy số chứng từ từ stock.picking liên quan
                sale_orders = line.invoice_ids.mapped('invoice_line_ids.sale_line_ids.order_id')
                if sale_orders:
                    pickings = sale_orders.mapped('picking_ids')
                    if pickings:
                        picking = pickings[0]
                        voucher_number = picking.name or ''
                
                # Lấy người phụ trách từ sale order cho dữ liệu cũ chưa có user_id
                if sale_orders and not person_in_charge:
                    user = sale_orders[0].user_id
                    if user:
                        person_in_charge = user.name or ''
            
            # Chuyển đổi số lượng sang tấn (giả sử đơn vị hiện tại là kg, cần chia 1000)
            # Nếu đơn vị đã là tấn thì giữ nguyên
            quantity_ton = line.quantity
            if line.product_uom_id:
                uom_name = line.product_uom_id.name or ''
                if 'KG' in uom_name.upper() or 'KG' in (line.product_uom_id.category_id.name or '').upper():
                    quantity_ton = line.quantity / 1000.0
                elif 'TẤN' in uom_name.upper() or 'TON' in uom_name.upper():
                    quantity_ton = line.quantity
            ...
            line_data = {
                ...
                "amount_paid": line.amount_paid or 0,
                "commission_amount": line.commission_amount or 0,
                "employee_commission_amount": line.employee_commission_amount or 0,
    ...
    def add_body(self, sheet, formats, obj, workbook):
        ...
        for region_name, region_data in lines_data.get("grouped", {}).items():
            ...
            if region_col_idx is not None and len(current_region_lines) > 1:
                ...
                region_format = formats[COLUMNS[region_col_idx].get('type', 'text')]
                sheet.merge_range(region_start_row, region_col_idx, region_end_row, region_col_idx, 
                                region_value, region_format)
