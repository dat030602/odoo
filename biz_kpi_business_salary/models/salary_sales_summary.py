...
class SalarySalesSummary(models.Model):
    ...
    humic_discount_ids = fields.One2many(
        string='Chiết khấu Humic',
        comodel_name='salary.humic.discount',
        inverse_name='summary_id'
    )
    ...
    sales_discount_ids = fields.One2many(
        string='Chiết khấu bán hàng',
        comodel_name='salary.sales.discount',
        inverse_name='summary_id'
    )
    ...
    def action_get_data(self):
        ...
        self.humic_discount_ids.unlink()
        ...
        self.sales_discount_ids.unlink()
        ...
        for team_id in team_ids:
            ...
            # Tính toán chiết khấu thương mại (SO lines có giá âm, không có stock.move)
            for src_line_id in current_team_line_ids:
                cur_sms = sms.filtered(lambda sm: sm.picking_id.sale_id.team_id == team_id and sm.picking_id.sale_id.order_state_id == src_line_id.state_id)
                discount_vals = src_line_id._prepare_discount_line_from_sale_orders(self.id, cur_sms, date_from, date_to)
                if discount_vals:
                    src_line_id.write({'detail_line_ids': [Command.create(val) for val in discount_vals]})
        ...
        self._create_humic_discount_data()
        self._create_commercial_discount_detail_data()
        self._create_sales_discount_data()
        self._create_personnel_commission_data()
        self._create_employee_commission_data()
    ...
    def _create_sales_discount_data(self):
        """Tạo dữ liệu chiết khấu bán hàng."""
        self.ensure_one()
        self.sales_discount_ids.unlink()
        self.env['salary.sales.discount'].create_default_lines(self.id)
    ...
    def _create_employee_commission_data(self):
        """Tạo dữ liệu tổng hợp hoa hồng nhân viên theo khoản mục."""
        self.ensure_one()
        self.employee_commission_ids.unlink()
        self.env['salary.employee.commission'].create_from_summary_tabs(self.id)
    ...
    def _create_personnel_commission_data(self):
        ...
        # Xóa dữ liệu cũ
        self.personnel_commission_ids.unlink()
        self.env['salary.personnel.commission'].create_from_summary_data(self.id)
    ...
    def _create_sales_commission_employee_data(self):
        ...
        # Tạo mặc định cho 4 nhân viên SS
    ...
    def _create_humic_discount_data(self):
        """Tạo dữ liệu chiết khấu Humic."""
        self.ensure_one()
        self.humic_discount_ids.unlink()
        self.env['salary.humic.discount'].create_default_lines(self.id)
    ...
    def _create_commission_result_data(self):
        ...
        self.ensure_one()
        self.commission_result_ids.unlink()
        self.env['salary.commission.result'].create_from_summary_tabs(self.id)
    ...
    def action_export_business_salary(self):
        self.ensure_one()
        return self.env.ref(
            'biz_kpi_business_salary.business_salary_report'
        ).report_action(self)

    @staticmethod
    def _set_payslip_line_amount(slip, amount, codes=(), keywords=()):
        codes = {code.upper() for code in codes}
        keywords = [keyword.lower() for keyword in keywords]
        lines = slip.line_ids.filtered(lambda line: (line.code or '').upper() in codes)
        if not lines and keywords:
            lines = slip.line_ids.filtered(
                lambda line: any(
                    keyword in ('%s %s' % (line.code or '', line.name or '')).lower()
        ...
        if not lines:
            return False
        line = lines[0]
        amount = -abs(amount) if line.total < 0 else amount
        line.write({
            'amount': amount,
            'quantity': 1.0,
            'rate': 100.0,
    ...
    def action_sync_may_2026_payslips(self):
        """Chốt các dòng chính của phiếu lương Kinh doanh theo bảng lương T5/2026."""
        self.ensure_one()
        if str(self.month) != '5' or int(self.year) != 2026:
            raise UserError('Chức năng này chỉ áp dụng cho bảng lương Kinh doanh tháng 5/2026.')

        report = self.env['report.biz_kpi_business_salary.rp_business_salary']
        rows, _standard_days = report._get_rows(self)
        date_from, date_to = self._get_date_range()
        ...
        payslip_by_employee = {slip.employee_id.id: slip for slip in payslips}
        employee_by_code = {
            (employee.code or '').strip().upper(): employee
            for employee in payslips.mapped('employee_id')
        }

        line_mapping = [
            (8, ('BASIC',), ('lương chuẩn', 'luong chuan', 'lương cơ bản', 'luong co ban')),
            (23, ('CT', 'LKPIDSHH'), ('hoa hồng', 'hoa hong', 'kpi ds')),
            (24, ('GROSS',), ('tổng lương', 'tong luong')),
            (32, ('TBH',), ('trừ bảo hiểm', 'tru bao hiem', 'tổng bảo hiểm')),
            (39, ('TNCN', 'THUETNCN', 'PIT'), ('thuế tncn', 'thue tncn')),
            (40, ('TTU',), ('tiền tạm ứng', 'tien tam ung', 'tạm ứng đợt 1', 'tam ung dot 1')),
            (42, ('KPCD', 'CD', 'CONGDOAN'), ('phí công đoàn', 'phi cong doan', 'công đoàn', 'cong doan')),
            (47, ('NET',), ('thực lãnh', 'thuc lanh', 'lương thực nhận', 'luong thuc nhan')),
        ]
        optional_line_mapping = [
            (11, (), ('lương ngày công', 'luong ngay cong')),
            (13, (), ('lương nghỉ lễ', 'luong nghi le')),
            (15, (), ('lương nghỉ phép', 'luong nghi phep')),
            (18, (), ('lương tăng ca', 'luong tang ca')),
            (19, (), ('ban điều hành', 'ban dieu hanh')),
            (20, (), ('ban công nghệ', 'ban cong nghe')),
            (21, (), ('phụ cấp xăng', 'phu cap xang')),
            (22, (), ('lương phí công tác', 'luong phi cong tac')),
            (25, (), ('lương đợt 1', 'luong dot 1')),
            (26, (), ('công tác đã thanh toán', 'cong tac da thanh toan')),
            (28, (), ('quà tết', 'qua tet')),
            (29, ('BHXH', 'BHXHNV', 'SI', 'SIN'), ('bhxh', 'bảo hiểm xã hội', 'bao hiem xahoi')),
            (30, ('BHYT', 'BHYTNV', 'HI', 'HIN'), ('bhyt', 'bảo hiểm y tế', 'bao hiem y te')),
            (31, ('BHTN', 'BHTNNV', 'UI', 'UIN'), ('bhtn', 'bảo hiểm thất nghiệp', 'bao hiem that nghiep')),
            (43, (), ('vi phạm', 'vi pham')),
            (44, (), ('chuyển khoản', 'chuyen khoan')),
            (45, (), ('truy thu',)),
            (46, (), ('bù lương', 'bu luong')),
        ]
        ...
        missing_lines = []
        for row in rows:
            employee_code = (row[1] or '').strip().upper()
            employee = employee_by_code.get(employee_code)
            slip = payslip_by_employee.get(employee.id) if employee else False
            if not slip:
                missing_payslips.append(employee_code or row[3])
                continue
            missing_for_employee = []
            for index, codes, keywords in line_mapping:
                if not self._set_payslip_line_amount(slip, row[index], codes, keywords):
                    missing_for_employee.append('/'.join(codes))
            for index, codes, keywords in optional_line_mapping:
                self._set_payslip_line_amount(slip, row[index], codes, keywords)
            if missing_for_employee:
                missing_lines.append('%s: %s' % (
                    employee_code,
                    ', '.join(missing_for_employee),
                ))
            synced += 1

        message = 'Đã đồng bộ %s phiếu lương Kinh doanh tháng 5/2026.' % synced
        if missing_payslips:
            message += ' Thiếu phiếu: %s.' % ', '.join(missing_payslips)
        if missing_lines:
            message += ' Thiếu dòng lương: %s.' % '; '.join(missing_lines)
        return {
            'type': 'ir.actions.client',
            'tag': 'display_notification',
            'params': {
                'title': 'Đồng bộ phiếu lương',
                'message': message,
                'type': 'warning' if missing_payslips or missing_lines else 'success',
                'sticky': bool(missing_payslips or missing_lines),
            },
        }
    ...
    def action_open_employee_commissions(self):
        return self._act_open('Hoa hồng nhân viên', 'salary.employee.commission', [('summary_id', '=', self.id)], search_default_group_team=False)
    ...
    def action_open_total_lines(self):
        return self._act_open('Tổng hợp theo khu vực', 'salary.sales.summary.total.line', [('summary_id', '=', self.id)])
    ...
    def push_salary_to_allowance(self):
        """Đẩy lương vào hr.allowance với code = CT (chỉ 80% hoa hồng)"""
        self.ensure_one()
        ...
        # Lấy tất cả commission results có số tiền > 0 và có nhân viên
        commission_results = self.commission_result_ids.filtered(lambda x: x.amount_80 > 0 and x.employee_id)
        
        if not commission_results:
            return
        
        # Nhóm theo employee_id — chỉ lấy 80% (thực nhận trong tháng)
        employee_results = {}
        for result in commission_results:
            if result.employee_id.id not in employee_results:
                employee_results[result.employee_id.id] = 0
            employee_results[result.employee_id.id] += result.amount_80
