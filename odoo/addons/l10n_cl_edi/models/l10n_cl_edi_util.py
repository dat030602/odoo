...
class L10nClEdiUtilMixin(models.AbstractModel):
    ...
    def _sign_full_xml(self, message, digital_signature, uri, xml_type, is_doc_type_voucher=False):
        ...
        signed_info = self.env.ref('l10n_cl_edi.signed_info_template').render({
            'uri': '#{}'.format(uri),
        ...
        signature = self.env.ref('l10n_cl_edi.signature_template').render({
    ...
    def _get_signed_token(self, digital_signature, seed):
        token_xml = self.env.ref('l10n_cl_edi.token_template').render({'seed': seed})
    ...
    def _get_send_status_ws(self, mode, company_vat, track_id, token):
        return Client(SERVER_URL[mode] + 'QueryEstUp.jws?WSDL').service.getEstUp(company_vat[:-2], company_vat[-1], track_id, token)
