...
class FetchmailServer(models.Model):
    ...
    def _process_incoming_email(self, msg_txt):
        ...
        for attachment in attachments:
            ...
            if attachment_ext.lower() != '.xml' or not self._is_dte_email(attachment.content):
    ...
    def _check_document_number_exists_no_partner(self, document_number, document_type, company_id, vat):
        ...
        name = '{} {}'.format(document_type.doc_code_prefix, document_number)
        return self.env['account.move'].sudo().search_count(
            [('type', 'in', ['in_invoice', 'in_refund']), ('name', '=', name), ('partner_id', '=', False),
             ('narration', '=', vat), ('company_id', '=', company_id)]) > 0
    ...
    def _create_invoice_from_attachment(self, att_content, att_name, from_address, company_id):
        ...
        for dte_xml in xml_content.xpath('//ns0:DTE', namespaces=XML_NAMESPACES):
            ...
            if not partner:
                invoice_form.narration = self._get_dte_issuer_vat(xml_content) or ''
