...
class SaleSubscription(models.Model):
    ...
    def create(self, vals):
        ...
        if subscription.partner_id and subscription.in_progress:
    ...
    def write(self, vals):
        ...
        old_in_progress = {s.id: s.in_progress for s in self}
        ...
        for subscription in self:
            ...
            diff_in_progress = subscription.in_progress != old_in_progress[subscription.id]
            if diff_partner or diff_in_progress:
                if subscription.in_progress:
                    subscription.message_subscribe(subscription.partner_id.ids)
                if diff_partner or not subscription.in_progress:
