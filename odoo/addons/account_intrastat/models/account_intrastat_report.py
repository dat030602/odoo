...
class IntrastatReport(models.AbstractModel):
    ...
    def _create_intrastat_report_line(self, options, vals):
        ...
        columns = [{'name': c} for c in [
            ...
            vals['region_code'], vals['commodity_code'], vals['type'], vals['intrastat_product_origin_country'],
            vals['partner_vat'],
        ]]
        if options.get('intrastat_extended'):
            columns += [{'name': c} for c in [
    ...
    def _build_query(self, date_from, date_to, journal_ids, invoice_types=None, with_vat=False):
        ...
        select = f"""
                ...
                CASE WHEN {product_origin_country_query_part} IS NULL THEN 'QU' ELSE product_country.code END AS intrastat_product_origin_country,
                CASE WHEN partner_country.id IS NULL THEN 'QV999999999999' ELSE partner.vat END AS partner_vat
                """
        from_ = f'''
