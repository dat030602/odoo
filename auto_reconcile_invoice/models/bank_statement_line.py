...
class BankStatementLine(models.Model):
    ...
    sale_show_ids = fields.Many2many('sale.order', 'bank_statement_line_sale_order_show_rel', string='Đơn hàng', compute='_compute_partner_show_ids', store=True)
    partner_show_ids = fields.Many2many('res.partner', 'bank_statement_line_res_partner_show_rel', string='Khách hàng', compute='_compute_partner_show_ids', store=True)
    ...
    line_ids = fields.One2many('bank.statement.partner.line', 'parent_id', string='Dòng sổ phụ')
    ...
    def default_get(self, fields_list):
        defaults = super(BankStatementLine, self).default_get(fields_list)
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', _('New')) == _('New'):
                vals['name'] = self.env['ir.sequence'].next_by_code('bank.statement.line') or _('New')
    ...
    def write(self, vals):
        ...
        if 'sale_ids' in vals:
            for rec in self:
                ...
                if payments:
                    payments.write({'sale_ids': [(6, 0, rec.sale_ids.ids)]})
                    # Tự động quét và đối soát cho các đơn mới được thêm
