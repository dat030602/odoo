...
(function (factory) {
  ...
  OrgChart.prototype = {
    ...
    panStartHandler: function (e) {
      ...
      var $chartContainer = $(e.delegateTarget);
      var $chart = $chartContainer.children();
      ...
      $chartContainer.on('mousemove touchmove',function(e) {
    ...
    panEndHandler: function (e) {
      if (e.data.chart.data('panning')) {
        e.data.chart.data('panning', false).css('cursor', 'default')
        e.data.chart.parent().off('mousemove');
      }
    },
    //
    bindPan: function () {
      ...
      $(document).on('mouseup mouseleave touchend', { 'chart': this.$chart }, this.panEndHandler);
    },
    //
    unbindPan: function () {
      this.$chartContainer.css('overflow', 'auto');
      this.$chartContainer.off('mousedown touchstart', this.panStartHandler);
      $(document).off('mouseup mouseleave touchend', this.panEndHandler);
    },
    //
    ...
    autoCentering: function() {
        var lastTf = this.$chart.css('transform');
        if (lastTf !== 'none') {
          var matrix = lastTf.split(',');
          if (lastTf.indexOf('3d') === -1) {
            var currentScale = window.parseFloat(matrix[3]);
            this.$chart.css('transform', 'matrix(' + currentScale + ', 0, 0, ' + currentScale + ', 0, 0)');
          } else {
            var currentScale = window.parseFloat(matrix[1]);
            this.$chart.css('transform', 'matrix3d(' + currentScale + ', 0, 0, 0, 0, ' + currentScale + ', 0, 0, 0, 0, ' + currentScale + ', 0, 0, 0, 0, 1)');
          }
        }
    },
    //
