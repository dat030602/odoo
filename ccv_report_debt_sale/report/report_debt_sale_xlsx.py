...
COLUMNS = [
    ...
    {"size": 10,                                "name": 'Số chứng từ'         , 'field':"name"                      , "type": 'text'               , "sum": False  },
    {"size": 10,                                "name": 'Số hoá đơn'          , 'field':"invoice_number"            , "type": 'text'               , "sum": False  },
    ...
    {"size": 10,                                "name": 'Mã hàng'             , 'field':"product_code"              , "type": 'text'               , "sum": False  },
    {"size": 15,                                "name": 'Tên hàng'            , 'field':"product_name"              , "type": 'text'               , "sum": False  },
    {"size":  8,                                "name": 'ĐVT'                 , 'field':"product_uom_id"            , "type": 'text'               , "sum": False  },
    {"size": 10,  "group": "Bán hàng",          "name": 'Số lượng bán'        , 'field':"quantity"                  , "type": 'number_c_float3'    , "sum": True   },
    {"size": 10,  "group": "Bán hàng",          "name": 'Đơn giá'             , 'field':"price_unit"                , "type": 'number_int'         , "sum": False  },
    {"size": 10,  "group": "Bán hàng",          "name": 'DS chưa thuế'        , 'field':"price_subtotal"            , "type": 'number_int'         , "sum": True  },
    {"size": 10,  "group": "Bán hàng",          "name": 'Thuế VAT'            , 'field':"price_tax"                 , "type": 'number_int'         , "sum": True  },
    {"size": 10,  "group": "Bán hàng",          "name": 'DS sau thuế'         , 'field':"price_total"               , "type": 'number_int'         , "sum": True  },
    ...
    {"size": 10,                                "name": 'Khu vực'             , 'field':"team_name"                 , "type": 'text'               , "sum": False  },
    {"size": 10,                                "name": 'Đơn hàng'            , 'field':"order_name"                , "type": 'text'               , "sum": False  },
]
...
def create_formats(workbook):
    return {
        ...
        "text_bold": format_workbook(workbook, 11.5, border=1, align='center', text_wrap=True, bold=True),
...
class ReportDebtSaleXlsx(models.AbstractModel):
    _name = "report.ccv_report_debt_sale.report_debt_sale_xlsx"
    ...
    def _get_data_export(self, obj):
        ...
        partner_ids = obj.line_ids.mapped('partner_id')
        for partner_id in partner_ids:
            lines = obj.line_ids.filtered(lambda l: l.partner_id == partner_id)
            count = 0
            lines_data = []
            sums = init_sums()
            for line in lines:
                count += 1
                vat_sinvoice_number = safe_data(line, "vat_sinvoice_number")
                data = {
                    "no": count,
                    "name":            safe_data(line, "name"),
                    "invoice_number":  vat_sinvoice_number.replace(',','\n') if vat_sinvoice_number else vat_sinvoice_number,
                    "date":            safe_data(line, "date"),
                    "partner_code":    safe_data(line, "partner_id", "code_contact"),
                    "partner_name":    safe_data(line, "partner_id", "name"),
                    "product_code":    safe_data(line, "product_id", "split_default_code"),
                    "product_name":    safe_data(line, "product_name"),
                    "product_uom_id":  safe_data(line, "product_uom", "name"),
                    "quantity":        safe_data(line, "qty_invoiced", is_num=True),
                    "price_unit":      safe_data(line, "price_unit", is_num=True),
                    "price_subtotal":  safe_data(line, "price_subtotal", is_num=True),
                    "price_tax":       safe_data(line, "amount_tax", is_num=True),
                    "price_total":     safe_data(line, "price_subtotal_tax", is_num=True),
                    "debt_in":         safe_data(line, "debt_in", is_num=True),
                    "debt_end":        safe_data(line, "debt_end", is_num=True),
                    "user_name":       safe_data(line, "user_id", "name_without_position"),
                    "team_name":       safe_data(line, "team_id", "code"),
                    "order_name":      safe_data(line, "order_id", "name"),
                }
                update_sums(sums, data)
                lines_data.append(data)
            
            if lines:
                grand_totals = update_sums(grand_totals, sums)
                grouped[partner_id.name] = {
                    "lines": lines_data,
                    "sums": sums
                }
    ...
    def add_body(self, sheet, formats, obj):
        ...
        for partner_name, partner_data in lines_data.get("grouped", {}).items():
            ...
            sheet.merge_range(prod_row, 0, prod_row, len(COLUMNS) - 1,
                              f"KHÁCH HÀNG: {partner_name.upper()}", formats["text_bold"])
            prod_row += 1
    ...
    def generate_xlsx_report(self, workbook: xlsxwriter.Workbook, obj, objects):
        for obj in objects:
            obj = obj.with_context(lang=self.env.user.lang)
