...
TITLE = 'Báo cáo thu tiền bốc xếp'
...
class bao_cao_san_luong_nhap_xuat_bocxep(models.AbstractModel):
    _name = "report.ccv_api_connector.bao_cao_san_luong_nhap_xuat_bocxep"
    ...
    def _get_data_export(self, obj):
        ...
        if obj.department_ids:
            line_ids = line_ids.filtered(lambda l: l.loading_id.id in obj.department_ids.ids or l.loading2_id.id in obj.department_ids.ids)
        ...
        for line_id in line_ids:
            ...
            if obj.department_ids:
                is_loading2_id = line_id.loading2_id.id in obj.department_ids.ids
                is_loading1_id = line_id.loading_id.id in obj.department_ids.ids
    ...
    def add_title(self, sheet, formats, obj):
        ...
        row += 1
        title_text = "Phòng ban: %s" % (", ".join(obj.department_ids.mapped("name")) if obj.department_ids else "Tất cả")
...
# COLUMNS mapping for rp.san.luong.nhap.xuat.bocxep (uses uom_name instead of uom_id)
COLUMNS_RP = [
    ...
    {"size": 15,  "name": 'Tiền công bốc xếp/ Tấn', 'field': "tien_bocxep_tan", "type": 'number_int', "sum": False},
    {"size": 15,  "name": 'Thành tiền'         , 'field': "thanh_tien"   , "type": 'number_int', "sum": True},
]


class rp_san_luong_nhap_xuat_bocxep_report(models.AbstractModel):
    _name = "report.ccv_api_connector.rp_san_luong_nhap_xuat_bocxep_xlsx"
    _inherit = "report.report_xlsx.abstract"
    _description = TITLE
    ...
    def _get_data_export(self, obj):
        ...
        for line in obj.line_ids.sorted('no'):
            val = {
                ...
                'note': line.note or '',
                'tien_bocxep_tan': line.tien_bocxep_tan or 0.0,
