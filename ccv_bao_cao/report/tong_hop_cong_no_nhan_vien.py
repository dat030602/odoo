...
TITLE = 'Tổng hợp công nợ nhân viên'

COLUMNS = [
    {"size": 5, "name": 'STT', 'field': "no", "type": 'number_c_int', "sum": False},
    {"size": 12, "name": 'Mã nhân viên', 'field': "employee_code", "type": 'text', "sum": False},
    {"size": 25, "name": 'Tên nhân viên', 'field': "employee_name", "type": 'text', "sum": False},
    ...
    {"size": 10, "group": "Số phát sinh", "name": 'Nợ', 'field': "debit", "type": 'number_int', "sum": True},
    {"size": 10, "group": "Số phát sinh", "name": 'Có', 'field': "credit", "type": 'number_int', "sum": True},
...
class TongHopCongNoNhanVien(models.AbstractModel):
    _name = "report.ccv_bao_cao.tong_hop_cong_no_nhan_vien"
    _inherit = "report.report_xlsx.abstract"
    _description = TITLE
    
    def _get_data_export(self, obj):
        """Get organized data for export - using line_ids from alpha.report.line"""
        lines_obj = obj.line_ids.sudo().filtered(lambda l:l.start_debit > 0 or l.start_credit > 0 or l.debit > 0 or l.credit > 0 or l.end_debit > 0 or l.end_credit > 0)
        count = 0
        lines = []
        ...
        for data in lines_obj:
            count += 1
            employee_name = data.employee_id.user_id.name_without_position or data.employee_id.name or ""
            vals = {
                "no": count,
                "employee_code": safe_data(data, "employee_code"),
        ...
        return {
            ...
            "title": "",
            "reporter": obj.sudo().env.user.name_without_position if hasattr(obj.sudo().env.user, 'name_without_position') else obj.sudo().env.user.name,
            "signature": self._get_signature(obj),
        }

    def _get_signature(self, obj):
        """Get signature data from alpha report"""
        voter_id = obj.voter_id.name_without_position if obj.voter_id and hasattr(obj.voter_id, 'name_without_position') else (obj.voter_id.name if obj.voter_id else "")
        chief_acc_id = obj.chief_acc_id.name_without_position if obj.chief_acc_id and hasattr(obj.chief_acc_id, 'name_without_position') else (obj.chief_acc_id.name if obj.chief_acc_id else "")
        unit_heads_id = obj.unit_heads_id.name_without_position if obj.unit_heads_id and hasattr(obj.unit_heads_id, 'name_without_position') else (obj.unit_heads_id.name if obj.unit_heads_id else "")
        return {
            'voter_id': voter_id,
    ...
    def add_title(self, sheet, formats, obj):
        ...
        subtitle = f"Từ ngày {obj.date_from.strftime('%d/%m/%Y')} đến ngày {obj.date_to.strftime('%d/%m/%Y')}"
    ...
    def add_body(self, sheet, formats, obj):
        ...
        all_lines = self._get_data_export(obj).get("all_lines", [])
        ...
        grand_totals = self._get_data_export(obj).get("grand_totals", {})
    ...
    def add_signatures(self, sheet, formats, obj):
        ...
        signature_blocks = [
            {"role": "Người bàn giao", "signer": obj.voter_id and obj.voter_id.name_without_position or obj.voter_id.name or ""},
            {"role": "Thủ kho", "signer": ""},
            {"role": "Kế Toán Trưởng", "signer": obj.chief_acc_id and obj.chief_acc_id.name_without_position or obj.chief_acc_id.name or ""},
            {"role": "Thủ trưởng đơn vị", "signer": obj.unit_heads_id and obj.unit_heads_id.name_without_position or obj.unit_heads_id.name or ""},
        ]
