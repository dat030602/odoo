...
TITLE = 'Chi tiết công nợ phải thu USD'
...
COLUMNS = [
    ...
    {"size": 10,                                "name": 'Ngày hóa đơn'        , 'field':"invoice_date"                     , "type": 'date'               , "sum": False  },
    {"size": 15,                                "name": 'Số hóa đơn'          , 'field':"reference"                        , "type": 'text'               , "sum": False  },
    {"size": 7,                                 "name": 'Số lượng'            , 'field':"product_uom_quantity"             , "type": 'number_float3'      , "sum": True  },
    {"size": 10,                                "name": 'Đơn giá'             , 'field':"price_unit"                       , "type": 'number_float2'      , "sum": False  },
    ...
    {"size": 10,  "group": "Dư Nợ cuối kỳ",    "name": 'Số tiền'              , 'field':"end_debit_usd"                    , "type": 'number_float2'      , "sum": True  , "value_end": True },
    {"size": 10,  "group": "Dư Nợ cuối kỳ",    "name": 'Quy đổi'              , 'field':"end_debit_vnd"                    , "type": 'number_int'         , "sum": True  , "value_end": True },
    {"size": 10,  "group": "Dư Có cuối kỳ",    "name": 'Số tiền'              , 'field':"end_credit_usd"                   , "type": 'number_float2'      , "sum": True  , "value_end": True },
    {"size": 10,  "group": "Dư Có cuối kỳ",    "name": 'Quy đổi'              , 'field':"end_credit_vnd"                   , "type": 'number_int'         , "sum": True  , "value_end": True },
    {"size": 5,                                 "name": 'Mã hàng'             , 'field':"default_code"                      , "type": 'text'               , "sum": False  },
]
...
class chi_tiet_cong_no_phai_thu_usd(models.AbstractModel):
    _name = "report.ccv_bao_cao.chi_tiet_cong_no_phai_thu_usd"
    _inherit = "report.report_xlsx.abstract"
    _description = TITLE

    def _get_data_export(self, obj):
        """Get organized data for export with grouping by partner"""
        grouped = {}
        grand_totals = init_sums()
        
        lines_obj = obj.beta_line6_ids.sudo()
        ...
        for partner_id in partner_ids:
            ...
            if line_ids:
                ...
                for data in line_ids:
                    ...
                    vals = {
                        "no": count,
                        "partner_name":          safe_data(partner_id, "name", sub_key="display_name"),
                        "date":                  safe_data(data, "date"),
                        "move_id":               safe_data(data, "move_id", sub_key="display_name"),
                        "invoice_date":          safe_data(data, "invoice_date"),
                        "reference":             safe_data(data, "reference"),
                        "product_uom_quantity":  safe_data(data, "product_uom_qty", is_num=True),
                        "price_unit":            safe_data(data, "price_unit"),
                        ...
                        "default_code":          safe_data(data, "default_code"),
                    }
                    lines.append(vals)
    ...
    def add_title(self, sheet, formats, obj):
        ...
        sheet.merge_range("C2:S2","Công ty TNHH Con Cò Vàng - Mã số thuế: 0305995751",formats["company_name"],)
    ...
    def add_body(self, sheet, formats, obj):
        ...
        # Lặp qua từng partner trong dữ liệu
        for partner_name, partner_data in lines_data.get("grouped", {}).items():
            # Ghi tiêu đề tên partner
            sheet.merge_range(prod_row, 0, prod_row, len(COLUMNS) - 1,
                              f"KHÁCH HÀNG: {partner_name.upper()}", formats["total_text_bold"])
            prod_row += 1

            # Ghi các dòng dữ liệu chi tiết cho partner hiện tại
            ...
            # Ghi dòng tổng cộng cho partner hiện tại (Sub-Total)
            ...
            # Ghi tổng của từng cột summable cho partner hiện tại
            for col_idx, column_def in enumerate(COLUMNS):
                if column_def['sum']:
                    field_name = column_def.get('field')
        ...
        # Ghi tổng của từng cột summable cho tất cả các partner
        for col_idx, column_def in enumerate(COLUMNS):
            if column_def['sum']:
                field_name = column_def.get('field')
