...
TITLE = 'Tổng hợp công nợ phải thu USD'

COLUMNS = [
    {"size": 5, "name": 'STT', 'field': "no", "type": 'number_c_int', "sum": False},
    {"size": 12, "name": 'Mã khách hàng', 'field': "customer_code", "type": 'text', "sum": False},
    {"size": 25, "name": 'Tên khách hàng', 'field': "customer_name", "type": 'text', "sum": False},
    {"size": 25, "name": 'Địa chỉ', 'field': "address", "type": 'text', "sum": False},
    {"size": 15, "name": 'Mã số thuế', 'field': "vat", "type": 'text', "sum": False},
    {"size": 10, "name": 'Tài khoản công nợ', 'field': "account_id", "type": 'text', "sum": False},
    {"size": 10, "group": "Dư Nợ đầu kỳ", "name": 'Số tiền', 'field': "start_debit_nt", "type": 'number_float2', "sum": True},
    {"size": 10, "group": "Dư Nợ đầu kỳ", "name": 'Quy đổi', 'field': "start_debit", "type": 'number_int', "sum": True},
    {"size": 10, "group": "Dư Có đầu kỳ", "name": 'Số tiền', 'field': "start_credit_nt", "type": 'number_float2', "sum": True},
    {"size": 10, "group": "Dư Có đầu kỳ", "name": 'Quy đổi', 'field': "start_credit", "type": 'number_int', "sum": True},
    {"size": 10, "group": "Phát sinh Nợ", "name": 'Số tiền', 'field': "ps_debit_nt", "type": 'number_float2', "sum": True},
    {"size": 10, "group": "Phát sinh Nợ", "name": 'Quy đổi', 'field': "ps_debit", "type": 'number_int', "sum": True},
    {"size": 10, "group": "Phát sinh Có", "name": 'Số tiền', 'field': "ps_credit_nt", "type": 'number_float2', "sum": True},
    {"size": 10, "group": "Phát sinh Có", "name": 'Quy đổi', 'field': "ps_credit", "type": 'number_int', "sum": True},
    {"size": 10, "group": "Dư Nợ cuối kỳ", "name": 'Số tiền', 'field': "end_debit_nt", "type": 'number_float2', "sum": True},
    {"size": 10, "group": "Dư Nợ cuối kỳ", "name": 'Quy đổi', 'field': "end_debit", "type": 'number_int', "sum": True},
    {"size": 10, "group": "Dư Có cuối kỳ", "name": 'Số tiền', 'field': "end_credit_nt", "type": 'number_float2', "sum": True},
    {"size": 10, "group": "Dư Có cuối kỳ", "name": 'Quy đổi', 'field': "end_credit", "type": 'number_int', "sum": True},
]
...
class tong_hop_cong_no_phai_thu_usd(models.AbstractModel):
    ...
    def _get_data_export(self, obj):
        """Get organized data for export without customer grouping - similar to production report"""
        lines_obj = obj.beta_line3_ids.sudo()
        count = 0
        lines = []
        ...
        for data in lines_obj:
            ...
            vals = {
                "no": count,
                "customer_code": safe_data(data, "customer_code"),
                "customer_name": safe_data(data, "customer_name"),
                "address": safe_data(data, "address"),
                "vat": safe_data(data, "vat"),
                "account_id": safe_data(data, "account_id", sub_key="code"),
                "start_debit_nt": safe_data(data, "start_debit_nt", is_num=True),
                "start_debit": safe_data(data, "start_debit", is_num=True),
                "start_credit_nt": safe_data(data, "start_credit_nt", is_num=True),
                "start_credit": safe_data(data, "start_credit", is_num=True),
                "ps_debit_nt": safe_data(data, "ps_debit_nt", is_num=True),
                "ps_debit": safe_data(data, "ps_debit", is_num=True),
                "ps_credit_nt": safe_data(data, "ps_credit_nt", is_num=True),
                "ps_credit": safe_data(data, "ps_credit", is_num=True),
                "end_debit_nt": safe_data(data, "end_debit_nt", is_num=True),
                "end_debit": safe_data(data, "end_debit", is_num=True),
                "end_credit_nt": safe_data(data, "end_credit_nt", is_num=True),
                "end_credit": safe_data(data, "end_credit", is_num=True),
            }
            lines.append(vals)
            
            # Update grand totals
            grand_totals = update_sums(grand_totals, vals, lines_obj)
        ...
        return {
            ...
            "title": obj.team_id.report_name if obj.partner_state in ('0','2') and obj.team_id else "",
            "reporter": obj.sudo().env.user.name_without_position,
            "team_lead": obj.sudo().team_id.user_id.name_without_position if obj.partner_state in ('0','2') and obj.team_id else "",
            "signature": self._get_signature(obj),
        }

    def _get_signature(self, obj):
        """Get signature data from alpha report"""
        voter_id = obj.voter_id.name_without_position if obj.voter_id else ""
        chief_acc_id = obj.chief_acc_id.name_without_position if obj.chief_acc_id else ""
        unit_heads_id = obj.unit_heads_id.name_without_position if obj.unit_heads_id else ""
        return {
            'voter_id': voter_id,
            'chief_acc_id': chief_acc_id,
            'unit_heads_id': unit_heads_id,
        }

    def add_title(self, sheet, formats, data):
        # Keep company name and address fixed
        sheet.merge_range("C2:R2","Công ty TNHH Con Cò Vàng - Mã số thuế: 0305995751", formats["company_name"])
        ...
        tile_report = data['data']['title']
        if tile_report:
            title_text += f" {tile_report.upper()}"
        ...
        account = account_account.code if account_account else ""

        subtitle = f"Tài khoản: {account}; Loại tiền: USD; Từ ngày {date_start} đến ngày {date_end}"
        sheet.merge_range(f"A5:{last_column_letter}5", subtitle, formats["title_sub"])
        sheet.set_row(4, 20)

    def add_header(self, sheet, formats, data):
        row_group = 6
        row_detail = 7
    ...
    def add_body(self, sheet, formats, data):
        ...
        # Ghi các dòng dữ liệu chi tiết (không nhóm theo khách hàng)
    ...
    def add_signatures(self, sheet, formats, data):
        ...
        lines_data = data['data']
        tile_report = lines_data.get("title", "")
        signature = lines_data.get("signature", {})
        
        if tile_report:
            signature_blocks = [
                {"role": "Người Lập Biểu", "signer": lines_data.get("reporter", "")},
                {"role": "Trưởng khu vực", "signer": lines_data.get("team_lead", "")},
                {"role": "Kế Toán Trưởng", "signer": signature.get('chief_acc_id', '')},
                {"role": "Thủ trưởng đơn vị", "signer": signature.get('unit_heads_id', '')},
            ]
        else:
            signature = lines_data.get("signature", {})
            signature_blocks = [
                {"role": "Người Lập Biểu", "signer": signature.get('voter_id', '')},
                {"role": "Kế Toán Trưởng", "signer": signature.get('chief_acc_id', '')},
                {"role": "Thủ trưởng đơn vị", "signer": signature.get('unit_heads_id', '')},
            ]
    ...
    def generate_xlsx_report(self, workbook: xlsxwriter.Workbook, data, objects):
        for obj in self.env['alpha.report'].browse(objects.ids):
            ...
            # Get data using the new _get_data_export method
            lines_data = self._get_data_export(obj)
            
            # Prepare data in the format expected by the report methods
            report_data = {
            ...
            self.add_title(sheet, formats, report_data)
            self.add_header(sheet, formats, report_data)
            self.add_body(sheet, formats, report_data)
            self.add_signatures(sheet, formats, report_data)
