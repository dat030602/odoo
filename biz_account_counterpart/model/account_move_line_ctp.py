...
class AccountMoveLineCTP(models.Model):
    ...
    dr_aml_id = fields.Many2one('account.move.line', string='Debit Move Line', required=True, ondelete='cascade', domain=_get_dr_aml_domain)
    dr_account_id = fields.Many2one(related='dr_aml_id.account_id', readonly=True, store=True, string="DR Account")
    dr_aml_debit = fields.Monetary(related='dr_aml_id.debit', currency_field='company_currency_id', readonly=True, store=True,
                                   help="The debit value of the debit journal item")
    cr_aml_id = fields.Many2one('account.move.line', string='Credit Move Line', required=True, ondelete='cascade', domain=_get_cr_aml_domain)
    cr_account_id = fields.Many2one(related='cr_aml_id.account_id', readonly=True, store=True, string="CR Account")
