...
class CreateSepayQRWizard(models.TransientModel):
    _name = 'create.sepay.qr.wizard'
    _description = 'Wizard to Create Sepay QR Code'

    sale_order_id = fields.Many2one('sale.order', string='Sale Order', required=True)

    amount = fields.Monetary(string='Amount')
    bank_id = fields.Many2one('sepay.bank', string='Bank')
    description = fields.Char(string='Description', default='Payment for Sale Order')
    currency_id = fields.Many2one('res.currency', string='Currency')

    def default_get(self, fields):
        res = super(CreateSepayQRWizard, self).default_get(fields)
        bank = self.env['sepay.bank'].search([('is_default', '=', True)], limit=1)
        if bank:
            res.update({
                'bank_id': bank.id,
            })
