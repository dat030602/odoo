...
class HrEmployee(models.Model):
    ...
    created_from_attendance_device = fields.Boolean(string='Created from Device', readonly=False,
    ...
    def _get_unaccent_name(self):
        return self.name #self.env['to.base'].no_accent_vietnamese(self.name)
    ...
    def create_device_user_if_not_exist(self, device):
        ...
        if not user:
            ...
            if device.unique_uid:
                update_vals.update({'user_id': self.barcode})
            else:
                update_vals.update({'uid': int(data['uid'])})
            user.write(update_vals)
        return user

    def action_load_attendance_device(self, device):
        self.ensure_one()
        if not self.barcode:
    ...
    def action_view_finger_template(self):
        action = self.env.ref('biz_attendance_device.action_finger_template')
...
class HrEmployeePublic(models.Model):
    ...
    created_from_attendance_device = fields.Boolean(string='Created from Device')