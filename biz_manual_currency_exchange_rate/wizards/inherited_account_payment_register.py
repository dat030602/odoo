...
class srAccountPaymentRegister(models.TransientModel):
    ...
    def onchange_inverse_manural_currency_exchange_rate(self):
        if self.inverse_manural_currency_exchange_rate and self.inverse_manural_currency_exchange_rate >= 0:
            self.manual_currency_exchange_rate = 1 / self.inverse_manural_currency_exchange_rate
    ...
    def default_get(self, fields):
        ...
        if self._context.get('active_model') == 'account.move':
            ...
            move_line_id = self.env['account.move.line'].browse(self._context.get('active_ids')).filtered(lambda move: move.move_id.is_invoice(include_receipts=True))
