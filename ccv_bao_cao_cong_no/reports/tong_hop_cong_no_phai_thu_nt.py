...
class tong_hop_cong_no_phai_thu_nt(models.AbstractModel):
    _name = "report.ccv_bao_cao_cong_no.tong_hop_cong_no_phai_thu_nt"
    _inherit = "report.report_xlsx.abstract"
    _description = "Tong Hop Cong No Phai Thu"

    def get_data_export(self, obj):
        lines_obj = obj.line1_th_nt_ids
        ...
        for data in lines_obj:
            ...
            vals = {
                ...
                "start_debit_usd": round(start_debit_usd, 2) if start_debit_usd != 0 else "",
                "start_debit_vnd": round(start_debit_vnd) if start_debit_vnd != 0 else "",
                "start_credit_usd": round(start_credit_usd, 2) if start_credit_usd != 0 else "",
                "start_credit_vnd": round(start_credit_vnd) if start_credit_vnd != 0 else "",
                "ps_debit_usd": round(ps_debit_usd, 2) if ps_debit_usd != 0 else "",
                "ps_debit_vnd": round(ps_debit_vnd) if ps_debit_vnd != 0 else "",
                "ps_credit_usd": round(ps_credit_usd, 2) if ps_credit_usd != 0 else "",
                "ps_credit_vnd": round(ps_credit_vnd) if ps_credit_vnd != 0 else "",
                "end_debit_usd": round(end_debit_usd, 2) if end_debit_usd != 0 else "",
                "end_debit_vnd": round(end_debit_vnd) if end_debit_vnd != 0 else "",
                "end_credit_usd": round(end_credit_usd, 2) if end_credit_usd != 0 else "",
                "end_credit_vnd": round(end_credit_vnd) if end_credit_vnd != 0 else "",
            }
            count += 1
            lines.append(vals)
        return {
            "sum": {
                "sum_start_credit_vnd": round(sum_start_credit_vnd) if sum_start_credit_vnd != 0 else sum_start_credit_vnd,
                "sum_start_debit_vnd": round(sum_start_debit_vnd) if sum_start_debit_vnd != 0 else sum_start_debit_vnd,
                "sum_ps_credit_vnd": round(sum_ps_credit_vnd) if sum_ps_credit_vnd != 0 else sum_ps_credit_vnd,
                "sum_ps_debit_vnd": round(sum_ps_debit_vnd) if sum_ps_debit_vnd != 0 else sum_ps_debit_vnd,
                "sum_end_credit_vnd": round(sum_end_credit_vnd) if sum_end_credit_vnd != 0 else sum_end_credit_vnd,
                "sum_end_debit_vnd": round(sum_end_debit_vnd) if sum_end_debit_vnd != 0 else sum_end_debit_vnd,
                "sum_start_credit_usd": round(sum_start_credit_usd, 2) if sum_start_credit_usd != 0 else sum_start_credit_usd,
                "sum_start_debit_usd": round(sum_start_debit_usd, 2) if sum_start_debit_usd != 0 else sum_start_debit_usd,
                "sum_ps_credit_usd": round(sum_ps_credit_usd, 2) if sum_ps_credit_usd != 0 else sum_ps_credit_usd,
                "sum_ps_debit_usd": round(sum_ps_debit_usd, 2) if sum_ps_debit_usd != 0 else sum_ps_debit_usd,
                "sum_end_credit_usd": round(sum_end_credit_usd, 2) if sum_end_credit_usd != 0 else sum_end_credit_usd,
                "sum_end_debit_usd": round(sum_end_debit_usd, 2) if sum_end_debit_usd != 0 else sum_end_debit_usd,
            },
            "lines": lines,
        }
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        for obj in objects.sudo():
            ...
            # Đầu đề
            title = "TỔNG HỢP CÔNG NỢ PHẢI THU%s" % ((" %s" % tile_report.upper()) if tile_report else " ")
            ...
            image_path = get_module_resource("ccv_bao_cao_cong_no", r"static/src/img", "logo.png")
