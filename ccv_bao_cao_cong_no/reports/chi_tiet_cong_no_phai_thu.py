...
class chi_tiet_cong_no_phai_thu(models.AbstractModel):
    _name = "report.ccv_bao_cao_cong_no.chi_tiet_cong_no_phai_thu"
    _inherit = "report.report_xlsx.abstract"
    _description = "Chi Tiet Cong No Phai Thu"
    
    def get_data_export(self, obj):
        lines_obj = obj.line1_ids
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        for obj in objects.sudo():
            ...
            # Đầu đề
            sheet.merge_range("A4:O4","CHI TIẾT CÔNG NỢ PHẢI THU%s" % ((" %s" % tile_report.upper()) if tile_report else " "),format1)
            sheet.set_row(4, 35)
            sheet.set_row(5, 20)
            ...
            header_arr = [
                ...
                {'name':"Số hóa đơn", "column": "D"},
                {'name':"Diễn giải", "column": "E"},
                {'name':"Tài khoản đối ứng", "column": "F"},
                {'name':"ĐVT", "column": "G"},
                {'name':"Số lượng", "column": "H"},
                {'name':"Đơn giá", "column": "I"},
            ]
            i = 0
            for header_item in header_arr:
                ...
                i += 1
            header_arr = [
                {"name": "Phát sinh", "from":"J", "to":"K", "items": [{"name" : "Nợ"},{"name" : "Có"},]},
                {"name": "Số dư", "from":"L", "to":"M", "items": [{"name" : "Nợ"},{"name" : "Có"},]},
            ]
            for header_item in header_arr:
            ...
            sheet.merge_range(f"N{w_row_header}:N{w_row_header+1}", "Mã hàng", format_header)
            sheet.merge_range(f"O{w_row_header}:O{w_row_header+1}", "Mã đơn hàng", format_header)
            ...
            columns = [
                ...
                {"size": 10, "name": "uom_id",               "is_num": False},
                {"size": 10, "name": "product_uom_quantity", "is_num": False},
                {"size": 10, "name": "price_unit",           "is_num": True, "currency":"vnd"},
                ...
                {"size": 10, "name": "order_id",             "is_num": False},
            ]
            ...
            for each in data_raw["lines"]:
                ...
                if each['no'] != 'sum' and int(each['no']) == 0:
                    prod_row += 1
                    sheet.merge_range(f"A{prod_row}:O{prod_row}", each['partner'], total_font_size_text)
                elif each['no'] == 'sum':
                    prod_row += 1
                    sheet.merge_range(f"A{prod_row}:G{prod_row}", "Tổng công nợ khách %s" % each['partner'], total_font_size_text)
                    prod_col += 7
                    prod_row -= 1
                    sheet.write(prod_row, prod_col, each["product_uom_quantity"] if each["product_uom_quantity"] != 0.00 else "", total_font_size_3)
                    prod_col += 1
                    sheet.write(prod_row, prod_col, "", total_font_size_3)
                    prod_col += 1
            ...
            prod_row = prod_row + 2
            sheet.merge_range(f"K{prod_row}:O{prod_row}","Ngày ..... tháng ..... năm .........",footer_font_size_sign)
            ...
            if obj.is_sale:
                ...
                sheet.merge_range(f"G{prod_row}:I{prod_row}","Phòng Kinh doanh",footer_font_size_title)
                sheet.merge_range(f"J{prod_row}:K{prod_row}","Kế toán trưởng",footer_font_size_title)
                sheet.merge_range(f"M{prod_row}:O{prod_row}","Thủ trưởng đơn vị",footer_font_size_title)
                ...
                sheet.merge_range(f"G{prod_row}:I{prod_row}","(Ký, họ tên)",footer_font_size_sign)
                sheet.merge_range(f"J{prod_row}:K{prod_row}","(Ký, họ tên)",footer_font_size_sign)
                sheet.merge_range(f"M{prod_row}:O{prod_row}","(Ký, họ tên, đóng dấu)",footer_font_size_sign)
                ...
                sheet.merge_range(f"G{prod_row}:I{prod_row}",obj.lead_sale_id.name_without_position if obj.lead_sale_id else "",footer_font_size_title)
                sheet.merge_range(f"J{prod_row}:K{prod_row}",obj.chief_acc_id.name_without_position if obj.chief_acc_id else "",footer_font_size_title)
                sheet.merge_range(f"M{prod_row}:O{prod_row}",obj.unit_heads_id.name_without_position if obj.unit_heads_id else "",footer_font_size_title)
            else:
                prod_row +=1
                ...
                sheet.merge_range(f"K{prod_row}:O{prod_row}","Thủ trưởng đơn vị",footer_font_size_title)
                ...
                sheet.merge_range(f"K{prod_row}:O{prod_row}","(Ký, họ tên, đóng dấu)",footer_font_size_sign)
                ...
                sheet.merge_range(f"K{prod_row}:O{prod_row}",obj.unit_heads_id.name_without_position if obj.unit_heads_id else "",  footer_font_size_title)
