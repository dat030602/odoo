...
class chi_tiet_cong_no_phai_tra_nt(models.AbstractModel):
    _name = "report.ccv_bao_cao_cong_no.chi_tiet_cong_no_phai_tra_nt"
    _inherit = "report.report_xlsx.abstract"
    _description = "Chi Tiet Cong No Phai Tra Ngoại Tệ"

    def get_data_export(self, obj):
        lines_obj = obj.line2_nt_ids
        ...
        for partner_id in partner_ids:
            ...
            if line_ids:
                for data in line_ids:
                    ...
                    count += 1
                    lines.append(vals)
                sum_line_ps_debit_nt    = sum(line_ids.mapped("ps_debit_nt"))
                sum_line_ps_debit       = sum(line_ids.mapped("ps_debit"))
                sum_line_ps_credit_nt   = sum(line_ids.mapped("ps_credit_nt"))
                sum_line_ps_credit      = sum(line_ids.mapped("ps_credit"))
                sum_line_end_debit_nt   = line_ids[-1].end_debit_nt
                sum_line_end_debit      = line_ids[-1].end_debit
                sum_line_end_credit_nt  = line_ids[-1].end_credit_nt
                sum_line_end_credit     = line_ids[-1].end_credit
                vals = {
                    "partner": data.partner_id.name,
                    "no": "sum",
                    "ps_debit_usd":     round(sum_line_ps_debit_nt, 2),
                    "ps_debit_vnd":     round(sum_line_ps_debit),
                    "ps_credit_usd":    round(sum_line_ps_credit_nt, 2),
                    "ps_credit_vnd":    round(sum_line_ps_credit),
                    "end_debit_usd":    round(sum_line_end_debit_nt, 2),
                    "end_debit_vnd":    round(sum_line_end_debit),
                    "end_credit_usd":   round(sum_line_end_credit_nt, 2),
                    "end_credit_vnd":   round(sum_line_end_credit),
                }
        ...
        return {
            "sum": {
                "sum_ps_credit_vnd":    round(sum_ps_credit_vnd) if sum_ps_credit_vnd != 0 else "",
                "sum_ps_debit_vnd":     round(sum_ps_debit_vnd) if sum_ps_debit_vnd != 0 else "",
                "sum_end_credit_vnd":   round(sum_end_credit_vnd) if sum_end_credit_vnd != 0 else "",
                "sum_end_debit_vnd":    round(sum_end_debit_vnd) if sum_end_debit_vnd != 0 else "",
                "sum_ps_credit_usd":    round(sum_ps_credit_usd, 2) if sum_ps_credit_usd != 0 else "",
                "sum_ps_debit_usd":     round(sum_ps_debit_usd, 2) if sum_ps_debit_usd != 0 else "",
                "sum_end_credit_usd":   round(sum_end_credit_usd, 2) if sum_end_credit_usd != 0 else "",
                "sum_end_debit_usd":    round(sum_end_debit_usd, 2) if sum_end_debit_usd != 0 else "",
            },
            "lines": lines,
        }
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        for obj in objects.sudo():
            ...
            # Đầu đề
            sheet.merge_range("A4:S4","CHI TIẾT CÔNG NỢ PHẢI TRẢ%s" % ((" %s" % tile_report.upper()) if tile_report else " "),format1)
            sheet.set_row(4, 35)
            sheet.set_row(5, 20)
