...
class BiSQLView(models.Model):
    ...
    def _prepare_search_view(self):
        ...
        filters_xml = "".join(
            [x._prepare_search_filter_field() for x in self.bi_sql_view_field_ids]
        )
        group_xml = (
            f"""<group expand="1" string="Group By">{filters_xml}</group>"""
            if filters_xml.strip()
            else ""
        )
        return {
            "name": self.name,
            ...
            """</search>""".format(fields_xml, group_xml),
        }
