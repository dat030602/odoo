...
class DetailBookAccount(models.AbstractModel):
    _name = 'detail.book.account.ccv'
    _inherit = 'account.report.custom.handler'
    _description = 'Detail of book account BTC (S38-DN) CCV'
    ...
    def _get_aml_values(self, report, options, expanded_account_ids, offset=0, limit=None):
        ...
        for aml_result in self._cr.dictfetchall():
            ...
            aml_result.update({
                ...
                'explain': aml_id.ref if aml_id.ref else aml_id.move_id.note,
    ...
    def export_to_xlsx(self, options, response=None):
        ...
        sheet.merge_range(y_offset, 12, y_offset, 13, "Mẫu số S38-DN", get_format({'bold': True, 'align': 'center'}))
        y_offset +=1
        ...
        sheet.merge_range(y_offset, 12, y_offset, 13, "(Ban hành theo Thông tư số 200/2014/TT-BTC", get_format({'align': 'center'}))
        y_offset +=1
        sheet.merge_range(y_offset, 12, y_offset, 13, "Ngày 22/12/2014 của Bộ Tài chính)", get_format({'align': 'center'}))
        y_offset +=1
        sheet.merge_range(y_offset, 0, y_offset, 13, "SỔ CHI TIẾT CÁC TÀI KHOẢN", get_format(report_title))
        y_offset +=1
        ...
        sheet.merge_range(y_offset, 0, y_offset , 13, sub_title, get_format(sub_title_format))
        y_offset +=1
        sheet.merge_range(y_offset, 0, y_offset, 13, "(Dùng cho các TK: 136, 138, 141, 157, 161, 171, 221, 222, 242, 244, 333, 334,", get_format(sub_title_format))
        y_offset +=1
        sheet.merge_range(y_offset, 0, y_offset, 13, "335, 336, 338, 344, 352, 353, 356,  411, 421, 441,  461, 466, ...)", get_format(sub_title_format))
        y_offset +=1
        sheet.merge_range(y_offset, 0, y_offset, 13, "Tài khoản: " + self._get_accounts(options), get_format(sub_title_format))
        y_offset +=1
        sheet.merge_range(y_offset, 0, y_offset, 13, "Đối tượng: " + self._get_partners(options), get_format(sub_title_format))
        y_offset +=1
        sheet.merge_range(y_offset, 0, y_offset, 13, "Loại tiền: VNĐ", get_format({'align': 'right','bold': True, 'italic': True}))
        y_offset +=1
        # Header talble
        sheet.write(y_offset,  0,  "Ngày hạch toán", get_format(header_format))
        sheet.write(y_offset,  1,  "Ngày chứng từ ", get_format(header_format))
        sheet.write(y_offset,  2,  "Số chứng từ", get_format(header_format))
        sheet.write(y_offset,  3,  "Ngày hoá đơn", get_format(header_format))
        sheet.write(y_offset,  4,  "Số hoá đơn", get_format(header_format))
        sheet.write(y_offset,  5,  "Diễn giải", get_format(header_format))
        sheet.write(y_offset,  6,  "Tài khoản", get_format(header_format))
        sheet.write(y_offset,  7,  "TK đối ứng", get_format(header_format))
        sheet.write(y_offset,  8,  "Phát sinh nợ", get_format(header_format))
        sheet.write(y_offset,  9,  "Phát sinh có ", get_format(header_format))
        sheet.write(y_offset,  10, "Dư nợ", get_format(header_format))
        sheet.write(y_offset,  11, "Dư có", get_format(header_format))
        sheet.write(y_offset,  12, "Mã đối tượng", get_format(header_format))
        sheet.write(y_offset,  13, "Tên đối tượng", get_format(header_format))
        y_offset+=1
        ...
        sheet.write(y_offset, 5, "F", get_format(header_format))
        sheet.write(y_offset, 6, "G", get_format(header_format))
        sheet.write(y_offset, 7, "H", get_format(header_format))
        sheet.write(y_offset, 8, "1", get_format(header_format))
        sheet.write(y_offset, 9, "2", get_format(header_format))
        sheet.write(y_offset, 10, "3", get_format(header_format))
        sheet.write(y_offset, 11, "4", get_format(header_format))
        sheet.write(y_offset, 12, "M", get_format(header_format))
        sheet.write(y_offset, 13, "N", get_format(header_format))
        y_offset +=1
        ...
        if print_options.get('hierarchy') is True:
            raise ValidationError(_("An error occurred with the new filter. Please select another filter to include in the report"))
        res = self.get_format_data_lines(report_id,lines)
        for line in res.values():
            if line['is_bold']:
                sheet.write(y_offset, 0, '', get_format(default_col1_style))
                sheet.write(y_offset, 1, '', get_format(default_col1_style))
                sheet.write(y_offset, 2, '', get_format(default_col1_style))
                sheet.write(y_offset, 3, '', get_format(default_col1_style))
                sheet.write(y_offset, 4, '', get_format(default_col1_style))
                sheet.write(y_offset, 5, line['d'] or '', get_format(level_2_style,{'align': 'left'}))
                sheet.write(y_offset, 6, line['a'] or '', get_format(level_2_style,{'align': 'center'}))
                sheet.write(y_offset, 7, line['e'] or '', get_format(level_2_style,{'align': 'right'}))
                sheet.write(y_offset, 8, line['1'], get_format(level_2_style,{'align': 'right'}))
                sheet.write(y_offset, 9, line['2'], get_format(level_2_style,{'align': 'right'}))
                sheet.write(y_offset, 10,line['3'], get_format(level_2_style,{'align': 'right'}))
                sheet.write(y_offset, 11,line['4'], get_format(level_2_style,{'align': 'right'}))
                sheet.write(y_offset, 12, '', get_format(level_2_style,{'align': 'right'}))
                sheet.write(y_offset, 13, '', get_format(level_2_style,{'align': 'right'}))
                y_offset +=1
            else:
                sheet.write(y_offset, 0, line['a'] or '', get_format(default_col1_style))
                sheet.write(y_offset, 1, line['c'] or '', get_format(default_col1_style))
                sheet.write(y_offset, 2, line['b'] or '', get_format(default_col1_style))
                sheet.write(y_offset, 3,  '', get_format(default_col1_style))
                sheet.write(y_offset, 4,  '', get_format(default_col1_style))
                sheet.write(y_offset, 5, line['d'] or '', get_format(default_col1_style))
                sheet.write(y_offset, 6, line['f'] , get_format(default_col1_style,{'align': 'center'}))
                sheet.write(y_offset, 7, line['e'] or '', get_format(default_col1_style,{'align': 'center'}))
                sheet.write(y_offset, 8, line['1'], get_format(default_col1_style,{'align': 'right'}))
                sheet.write(y_offset, 9, line['2'], get_format(default_col1_style,{'align': 'right'}))
                sheet.write(y_offset, 10, line['3'], get_format(default_col1_style,{'align': 'right'}))
                sheet.write(y_offset, 11, line['4'], get_format(default_col1_style,{'align': 'right'}))
                sheet.write(y_offset, 12, line['g'], get_format(default_col1_style))
                sheet.write(y_offset, 13, line['h'], get_format(default_col1_style))
                y_offset +=1
    ...
    def get_format_data_lines(self, report_id, lines):
        ...
        for parent_id, lines in group_accounts.items():  
            ...
            for line in lines:
                ...
                if line.get("class") == 'o_account_reports_initial_balance':
                    ...
                    res[y_offset] = {
                        ...
                        '1': '',
                        '2': '',
                        '3': initial_balance > 0 and initial_balance_str or 0,
                        '4': initial_balance < 0 and initial_balance_str or 0,
                        'is_bold': True
                    }
                    y_offset +=1      
                        ...
                        if float(vals['debit'] or 0) > 0 and not any(ed.debit > 0 for ed in another_line):
                            for edeb in another_line:
                                ...
                                res[y_offset] = {
                                    ...
                                    'f': account_name,
                                    'g': edeb.partner_id.code_contact if edeb.partner_id else '',
                                    'h': edeb.partner_id.name if edeb.partner_id else '',
                                    '1': self.format_float_number(edeb.credit),
                                    '2': self.format_float_number(edeb.debit),
                                    '3': initial > 0 and self.format_float_number(initial) or 0,
                                    '4': initial < 0 and self.format_float_number(abs(initial)) or 0,
                                    'is_bold': False
                                }
                        ...
                        if float(vals['credit'] or 0) > 0 and not any(ec.credit > 0 for ec in another_line):
                            for ecre in another_line:
                                ...
                                res[y_offset] = {
                                    ...
                                    'f': account_name,
                                    'g': ecre.partner_id.code_contact if ecre.partner_id else '',
                                    'h': ecre.partner_id.name if ecre.partner_id else '',
                    ...
                    res[y_offset] = {
                        ...
                        'f': account_name,
                        'g': aml_id.partner_id.code_contact if aml_id.partner_id else '',
                        'h': aml_id.partner_id.name if aml_id.partner_id else '',
            ...
            res[y_offset] = {
                ...
                '1': '',
                '2': '',
                '3': last_balance > 0 and self.format_float_number(abs(last_balance)) or 0,
                '4': last_balance < 0 and  self.format_float_number(abs(last_balance)) or 0,
                'is_bold': True
            }   
            y_offset +=1    
            ...
            if line.get('parent_id', False):
                ...
                if line['id'] not in parent_name:
                    if 'search_key' in line:
                        parent_name[line['id']] = line['search_key']
                    else:
                        parent_name[line['id']] = ''
    ...
    def acction_print_pdf(self, options):
        if options.get('hierarchy') is True:
            raise ValidationError(_("An error occurred with the new filter. Please select another filter to include in the report"))
        return self.env.ref("biz_details_of_account_books_ccv.action_print_detail_book_pdf_ccv").report_action(self, data=options)